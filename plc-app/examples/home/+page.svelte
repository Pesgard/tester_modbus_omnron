	...
	function startDataSimulation() {
		...
		dataInterval = setInterval(() => {
			if (isConnected && isRunning && productionData.processedItems < productionData.totalItems) {
				...
				productionData.accuracy = Math.round(
					(productionData.goodItems / productionData.processedItems) * 100
				);
		...
		<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
			...
			<div class="card space-y-4 p-6">
				...
				<div class="flex items-center gap-2">
					{#if isRunning}
						<div class="bg-success-500 h-3 w-3 animate-pulse rounded-full"></div>
			...
			<div class="card space-y-4 p-6">
				...
				<div>
					...
					<span class="badge {getEfficiencyColor(productionData.efficiency)} mt-2 text-xs">
		...
		<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
			...
			<div class="card space-y-6 p-6">
				...
				<div class="space-y-4">
					...
					<div class="bg-surface-300-600 h-4 w-full rounded-full">
						<div
				...
				<div class="grid grid-cols-2 gap-4">
					<div class="bg-success-500/10 rounded-lg p-4 text-center">
						<CheckCircle size={32} class="text-success-500 mx-auto mb-2" />
						<p class="text-success-500 text-2xl font-bold">{productionData.goodItems}</p>
						<p class="text-sm opacity-75">Good Items</p>
					</div>
					<div class="bg-error-500/10 rounded-lg p-4 text-center">
						<XCircle size={32} class="text-error-500 mx-auto mb-2" />
						<p class="text-error-500 text-2xl font-bold">{productionData.badItems}</p>
			...
			<div class="card space-y-6 p-6">
				...
				<div class="space-y-4">
					...
					<div class="bg-surface-300-600 h-4 w-full rounded-full">
						<div
							class="h-4 rounded-full transition-all duration-300 {productionData.accuracy >= 95
								? 'bg-success-500'
								: productionData.accuracy >= 85
									? 'bg-warning-500'
									: 'bg-error-500'}"
					...
					<div class="flex justify-between text-sm opacity-75">
						<span>Target: 95%</span>
						<span
							class={productionData.accuracy >= 95
								? 'text-success-500'
								: productionData.accuracy >= 85
									? 'text-warning-500'
									: 'text-error-500'}
						>
							{productionData.accuracy >= 95
								? 'Excellent'
								: productionData.accuracy >= 85
									? 'Good'
									: 'Needs Improvement'}
				...
				<!-- Quality Trend -->
				<div class="border-surface-300-600 grid grid-cols-3 gap-4 border-t pt-4">
