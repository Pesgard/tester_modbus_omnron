	...
	<div class="flex-1 overflow-auto p-4">
		<Tabs value={activeTab} onValueChange={(e) => (activeTab = e.value)}>
			{#snippet list()}
				...
				<button
					class="btn-icon preset-tonal ml-auto"
								...
								<tbody>
									{#each orders as order}
										<tr class="hover:preset-tonal">
											<td>
												<div class="space-y-2">
													<p class="text-sm font-bold">{order.lackType}</p>
													<p class="max-w-xs truncate text-xs opacity-60">{order.description}</p>
											...
											<td>
												<div class="space-y-1">
													...
													<div class="bg-surface-300-600 h-2 w-full rounded-full">
														<div
											...
											<td>
												<select
													...
													onchange={(e) =>
														updateOrderStatus(
															order.id,
															e.currentTarget.value as LackOrder['status']
														)}
												>
											...
											<td>
												<span
													class="badge {user.isActive
														...
														: 'preset-filled-surface-500'} text-xs"
												>
...
>
	{#snippet content()}
		<header class="flex items-center justify-between">
			...
			<button class="btn-icon preset-tonal" onclick={() => (showOrderModal = false)}>
				<X size={16} />
			</button>
		</header>
		...
		<footer class="flex justify-end gap-4">
			<button type="button" class="btn preset-tonal" onclick={() => (showOrderModal = false)}>
				Cancel
			</button>
			<button type="button" class="btn preset-filled" onclick={saveOrder}>
				<Save size={16} />
...
>
	{#snippet content()}
		<header class="flex items-center justify-between">
			...
			<button class="btn-icon preset-tonal" onclick={() => (showUserModal = false)}>
				<X size={16} />
			</button>
		</header>
		...
		<footer class="flex justify-end gap-4">
			<button type="button" class="btn preset-tonal" onclick={() => (showUserModal = false)}>
				Cancel
			</button>
			<button type="button" class="btn preset-filled" onclick={saveUser}>
				<Save size={16} />
